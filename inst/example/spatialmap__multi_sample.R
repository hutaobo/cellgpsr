...
# 逐个处理每个 sample
for (s in samplelist) {
  submeta <- subset(metadata, sample == s)
  ...
  plot_cophenetic_heatmap(row_coph,
                          figsize = c(10, 10),
                          matrix_name = "row_coph",
                          output_dir = "./output",
                          sample = s)
  plot_cophenetic_heatmap(col_coph,
                          figsize = c(10, 10),
                          matrix_name = "col_coph",
                          output_dir = "./output",
                          sample = s)
}
...
sample_names <- c("hd_samples", "la_samples", "ma_samples")
...
for (i in 1:length(sample_lists)) {
  ...
  # 绘制 col_cophenetic 热图
  plot_cophenetic_heatmap(
    matrix = average_col_cophenetic,
    matrix_name = "col_coph",
    figsize = c(12, 12),
    output_dir = "./output",
    sample = sample_name
